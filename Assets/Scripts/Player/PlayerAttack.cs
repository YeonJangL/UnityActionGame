...
public class PlayerAttack : MonoBehaviour
{
    ...
    public void NormalAttack()
    {
        ...
        foreach(var one in targetList)
        {
            ...
            if (enemy != null)
            {
                // 대미지 주고, 얼마나 뒤로 밀려날지 처리(pushback)
    ...
    public void SkillAttack()
    {
        ...
        foreach (var one in targetList)
        {
            ...
            // 3. 스킬 대미지 만큼 대미지를 주며 1초 딜레이 2만큼 pushback
            if (enemy != null)
            {
                StartCoroutine(enemy.StartDamage(NormalDamage, transform.position, 0.5f, 0.5f));
            }
    ...
    public void DashAttack()
    {
        ...
        foreach (var one in targetList)
        {
            ...
            // 3. 대쉬 대미지 만큼 대미지를 주며 1초 딜레이 2만큼 pushback
            if (enemy != null)
            {
                StartCoroutine(enemy.StartDamage(DashDamage, transform.position, 0.5f, 0.5f));
            }
        }
    }
}